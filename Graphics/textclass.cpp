	...
	D3DXMATRIX baseViewMatrix)
{
	...
	result = m_Font->Initialize(device, (char *)"../Graphics/data/fontdata.txt", (WCHAR *)L"../Graphics/data/font.dds");
	if (!result)
	{
	...
	// Initialize the first sentence.
	result = InitializeSentence(&m_sentence2, 100, device);
	if (!result)
	{
		return false;
	}
	...
	result = UpdateSentence(m_sentence3, (char *)"Goodbye", 100, 200, 1.0f, 1.0f, 0.0f, deviceContext);
	if (!result)
	{
		return false;
	}

	result = InitializeSentence(&m_sentence4, 100, device); //20200621
	if (!result)
	{
		return false;
	}

	result = UpdateSentence(m_sentence4, (char *)"Goodbye", 100, 200, 1.0f, 1.0f, 0.0f, deviceContext); //20200621
	if (!result)
	{
		return false;
	}
...
void TextClass::Shutdown()
{
	...
	ReleaseSentence(&m_sentence4); //20200621
...
bool TextClass::Render(ID3D11DeviceContext* deviceContext, D3DXMATRIX worldMatrix, D3DXMATRIX orthoMatrix)
{
	...
	result = RenderSentence(deviceContext, m_sentence4, worldMatrix, orthoMatrix); //20200621
	if (!result)
	{
		return false;
	}
...
bool TextClass::Count_polygons(int count, ID3D11DeviceContext* deviceContext)
{
	char tempString[30];
	char countString[30];
	bool result;
	...
	indexcount = count / 3;
	_itoa_s(indexcount, tempString, 10);

	strcpy_s(countString, "Polygons: ");
...
bool TextClass::SetScreen(int screenwidth, int screenheight, ID3D11DeviceContext *deviceContext) //20200621
{
	bool result;

	char* tempString = (char *) "screen size ";
	char String[100];

	sprintf_s(String, sizeof(String), "%s %d X %d", tempString, screenwidth, screenheight);

	// Update the sentence vertex buffer with the new string information.
	result = UpdateSentence(m_sentence4, String, 20, 80, 1.0f, 1.0f, 1.0f, deviceContext);
	if (!result)
	{
		return false;
	}
